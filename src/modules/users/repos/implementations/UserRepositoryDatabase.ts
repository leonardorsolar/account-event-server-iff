...
export default class UserRepositoryDatabase implements IUsersRepository {
  ...
  async save(user: User): Promise<any> {
    const conn = await this.connection.beginTransaction();
    //const instance_conn = await this.connection.beginTransaction();
    try {
      // const [userData] = await instance_conn.query("insert into schema_users.users (id_user, name, email, password) values ($1, $2, $3, $4) returning  *", [
      const [userData] = await conn.query("insert into schema_users.users (id_user, name, email, password) values ($1, $2, $3, $4) returning  *", [user.id_user, user.name, user.email, user.password]);
      console.log(userData);
      //inserir novo insert

      //  await instance_conn.commit();
      await this.connection.commit(conn);
      return userData;
    } catch (error) {
      //await instance_conn.roolback();
      await this.connection.rollback(conn);
      return error;
    }
  }
