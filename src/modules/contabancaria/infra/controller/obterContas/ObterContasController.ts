...
export default class ObterContasController {
    constructor(readonly servicoObterContas: IUseCase) {}
    ...
    async handle(req: Request, res: Response): Promise<Response> {
        console.log("ObterContasController")
        //console.log(req.query)
        try {
            const result = await this.servicoObterContas.execute(req?.body)
            console.log("retorno na controller ObterContasController")
            if (result.isLeft()) {
